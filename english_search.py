...
class FileTraverser():
	...
	def title_search(self, page_id):
		...
		title = linecache.getline('../output_data/english_wiki_index/id_title_map.txt', int(page_id)+1).strip()
	...
	def search_field_file(self, field, file_num, line_num):
		...
		if line_num != '':
			line = linecache.getline(f'../output_data/english_wiki_index/{field}_data_{str(file_num)}.txt', int(line_num)).strip()
	...
	def get_token_info(self, token):
		...
		if token[0] in char_list:
			with open(f'../output_data/english_wiki_index/tokens_info_{token[0]}_count.txt', 'r') as f:
			...
			with open(f'../output_data/english_wiki_index/tokens_info_others_count.txt', 'r') as f:
				num_tokens = int(f.readline().strip())

			tokens_info_pointer = f'../output_data/english_wiki_index/tokens_info_others.txt'
			token_info = self.binary_search_token_info(num_tokens, tokens_info_pointer, token)
...
if __name__=='__main__':
	...
	with open('../output_data/english_wiki_index/num_pages.txt', 'r') as f:
	...
	temp = linecache.getline('../output_data/english_wiki_index/id_title_map.txt', 0)
